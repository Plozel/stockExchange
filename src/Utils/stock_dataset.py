...
class StockExchangeDataset(Dataset):
    ...
    def preprocess_data(self):
        ...
        features = self.data.iloc[:, np.r_[26:79, 12:16]]
        ...
        real_y_2 = torch.tensor(labels.values).long()
...
class StockExchangeSeqDataset(Dataset):
    ...
    def preprocess_data(self):
        ...
        for idx in pd.unique(stocks_id):
            ...
            features_seq.append(grouped_idx.iloc[:, np.r_[2:59]].values.tolist())
            real_y_1_seq.append(grouped_idx.loc[:, 'class'].values.tolist())
